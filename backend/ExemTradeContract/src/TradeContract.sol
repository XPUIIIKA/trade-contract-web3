...
contract TradeContract {
    ...
    function createProducts(
        ...
        uint _price
    ) public onlySeller inState(State.Ready) {
        require(
            (totalCost() + _price) * 2 < insurance,
    ...
    function totalCost() private view returns (uint256) {
        uint256 total = 0;
    ...
    function payOrder() public payable onlyCustomer {
        require(
            msg.value == products[orderItem] * 2,
            "Order cost is not match!"
        );
    ...
    function confirmPayment() public onlySeller inState(State.OrderPaid) {
        state = State.Shipment;
    }

    function orderCompleted() public onlySeller inState(State.Shipment) {
        uint256 price = products[orderItem];
        require(price > 0, "Invalid product or already sold");
        require(
            address(this).balance >= price * 4,
